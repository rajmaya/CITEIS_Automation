...
public class HTMLDesignFilesJSWriter
{
  ...
  public static void pieChartJS(int paramInt1, int paramInt2, int paramInt3, int paramInt4)
  {
    try
    {
      PrintWriter localPrintWriter = new PrintWriter(Directory.RESULTSDir + Directory.SEP + Directory.RUNName + paramInt4 + Directory.SEP + "pieChart.js");
      localPrintWriter.println("$(document).ready(function() {\n    var data = [['" + ReportLabels.PASS.getLabel() + "', " + paramInt1 + "], ['" + ReportLabels.FAIL.getLabel() + "', " + paramInt2 + "], ['" + ReportLabels.SKIP.getLabel() + "', " + paramInt3 + "]];\n" + "    jQuery.jqplot('chart', [data],\n" + "            {seriesColors: [\"" + Colors.PASS.getColor() + "\", \"" + Colors.FAIL.getColor() + "\", \"" + Colors.SKIP.getColor() + "\"],\n" + "                seriesDefaults: {\n" + "                    // Make this a pie chart.\n" + "                    renderer: jQuery.jqplot.PieRenderer,\n" + "                    rendererOptions: {\n" + "                        padding: 15,\n" + "                        sliceMargin: 1,\n" + "                        // Put data labels on the pie slices.\n" + "                        // By default, labels show the percentage of the slice.\n" + "                        showDataLabels: true\n" + "                    }\n" + "                },\n" + "                grid: {borderColor: '#cccccc', background: '#ffffff',\n" + "                    borderWidth: 0, // pixel width of border around grid.\n" + "                    shadow: false // draw a shadow for grid.\n" + "                },\n" + "                legend: {show: true, location: 'e'}\n" + "            }\n" + "    );\n" + "});");
  ...
  public static void barChartJS(String paramString1, String paramString2, String paramString3, int paramInt)
  {
    ...
    try
    {
      ...
      localPrintWriter.print("];");
      localPrintWriter.println("    $.jqplot('bar', [s1, s2, s3], {\n        animate: true,axesDefaults:{min:0,tickInterval: " + TICK_INTERVAL + "" + "}," + "        seriesColors: [\"" + Colors.PASS.getColor() + "\", \"" + Colors.FAIL.getColor() + "\", \"" + Colors.SKIP.getColor() + "\"],\n" + "        stackSeries: false,\n" + "        seriesDefaults: {\n" + "            renderer: $.jqplot.BarRenderer,\n" + "            pointLabels: {show: true}\n" + "            , rendererOptions: {barWidth: 12, barMargin: 25, fillToZero: true}\n" + "        }\n" + "        ,\n" + "        grid: {borderColor: '#ffffff', background: '#ffffff',\n" + "            borderWidth: 0.5, // pixel width of border around grid.\n" + "            shadow: false // draw a shadow for grid.\n" + "        }\n" + "        ,\n" + "        legend: {\n" + "            show: true,\n" + "            location: 'e',\n" + "            placement: 'outside',\n" + "            labels: ['" + ReportLabels.PASS.getLabel() + "', '" + ReportLabels.FAIL.getLabel() + "', '" + ReportLabels.SKIP.getLabel() + "']\n" + "        },\n" + "        axes: {\n" + "            xaxis: {\n" + "                renderer: $.jqplot.CategoryAxisRenderer,\n" + "                ticks: ticks,\n" + "                label: \"" + ReportLabels.X_AXIS.getLabel() + "\"\n" + "            }\n" + "            ,\n" + "            yaxis: {\n" + "                label: \"" + ReportLabels.Y_AXIS.getLabel() + "\",\n" + "                tickOptions: {\n" + "                    formatString: \"%dtc\"\n" + "                }\n" + "            }\n" + "        }\n" + "    });\n" + "});");
      localPrintWriter.close();
    }
    catch (FileNotFoundException localFileNotFoundException)
    {
